...
def main():
    ...
    var_names = ['CMS_hgg_mass', 'whmva',
                 'dipho_lead_ptoM', 'dipho_sublead_ptoM', 'dipho_leadEta', 'dipho_subleadEta',
                 'CosPhi', 
                 ]
	...
	print '\n\nOn sample: %s' %sample
        ...
        for var in var_names:
          if ntup.GetEntries() == 0:
            ...
            if var == 'dipho_lead_ptoM':     histos[var][sample] = TH1D('%s_%s'%(var, sample), '', 40,   0,   4)
            if var == 'dipho_sublead_ptoM':  histos[var][sample] = TH1D('%s_%s'%(var, sample), '', 40,   0,   2)
            if var == 'dipho_leadEta':       histos[var][sample] = TH1D('%s_%s'%(var, sample), '', 60,  -3,   3)
            if var == 'dipho_subleadEta':    histos[var][sample] = TH1D('%s_%s'%(var, sample), '', 60,  -3,   3)
            if var == 'CosPhi':              histos[var][sample] = TH1D('%s_%s'%(var, sample), '', 40,  -1,   1)
            ...
            elif var == 'dipho_lead_ptoM':     ntup.Draw("%s >> %s_%s(40,  0,  4)"%(var, var, sample), final_wgt)
            elif var == 'dipho_sublead_ptoM':  ntup.Draw("%s >> %s_%s(40,  0,  2)"%(var, var, sample), final_wgt)
            elif var == 'dipho_leadEta':       ntup.Draw("%s >> %s_%s(60, -3,  3)"%(var, var, sample), final_wgt)
            elif var == 'dipho_subleadEta':    ntup.Draw("%s >> %s_%s(60, -3,  3)"%(var, var, sample), final_wgt)
            elif var == 'CosPhi':              ntup.Draw("%s >> %s_%s(40, -1,  1)"%(var, var, sample), final_wgt)
	...
	canv.Write()
        SaveCanvPic(canv, 'plots/MVA_vars', analyzer_cfg.channel + '_' + var_name)
